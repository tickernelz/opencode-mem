...
export abstract class OpenAIChatCompletionBaseProvider extends BaseAIProvider {
  protected constructor(
    config: ProviderConfig,
    protected readonly aiSessionManager: AISessionManager
  ) {
    super(config);
  }
  ...
  protected abstract getSessionProviderType(): AIProviderType;

  protected abstract getApiErrorLogLabel(): string;

  protected abstract getResponseBodyErrorLogLabel(): string;

  protected abstract getInvalidResponseLogLabel(): string;

  protected abstract getToolValidationLogLabel(): string;

  protected abstract buildRetryPrompt(toolSchema: ChatCompletionTool): string;
  ...
  async executeToolCall(
    ...
    let session = this.aiSessionManager.getSession(sessionId, this.getSessionProviderType());

    if (!session) {
      session = this.aiSessionManager.createSession({
        provider: this.getSessionProviderType(),
        sessionId,
      });
    }
    ...
    while (iterations < maxIterations) {
      ...
      try {
        ...
        if (data.status && data.msg) {
          log(this.getResponseBodyErrorLogLabel(), {
            provider: this.getProviderName(),
            model: this.config.model,
            status: data.status,
            msg: data.msg,
        ...
        if (!data.choices || !data.choices[0]) {
          log(this.getInvalidResponseLogLabel(), {
            provider: this.getProviderName(),
            model: this.config.model,
        ...
        const choice = (data as ToolCallResponse).choices[0];
        if (!choice) {
          return {
            success: false,
            error: "Invalid API response format",
          ...
          for (const toolCall of choice.message.tool_calls) {
            ...
            if (toolCall.function.name === toolSchema.function.name) {
              try {
                ...
                log(this.getToolValidationLogLabel(), {
        ...
        const retryPrompt = this.buildRetryPrompt(toolSchema);
